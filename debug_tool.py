"""
TFM DATA ENGINEERING - HERRAMIENTA DE DEBUGGING COMPREHENSIVA
            ...
            try:
                ...
                for attr in expected_attrs:
                    ...
                    if has_attr:
                        ...
                        self.logger.log(f"⚠️  {module_name}.{attr} no encontrado", 
                                      LogLevel.WARNING)
...
class PipelineChecker:
    ...
    def _parse_etl_output(self, output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if 'extracción' in line_lower or '📥' in line:
            ...
            elif 'transformación' in line_lower or '🔄' in line:
...
class SystemChecker:
    ...
    def check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result['error'] = str(e)
            self.logger.log(f"⚠️  Error verificando sistema: {e}", 
                          LogLevel.WARNING)
...
⚠️  Advertencias: {summary.get('warnings', 0)}
...
"""
        ingestion = self.results.get('ingestion_test', {})
        text += f"Ingesta: {'✅' if ingestion.get('success') else '❌'} "
        text += f"({ingestion.get('duration_seconds', 0)}s)\n"
        
        etl = self.results.get('etl_test', {})
        text += f"ETL Pipeline: {'✅' if etl.get('success') else '❌'} "
